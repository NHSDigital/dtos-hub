...
module "virtual-desktop" {
  ...
  source = "git::https://github.com/NHSDigital/dtos-devops-templates.git//infrastructure/modules/virtual-desktop?ref=25800a1f5bce5c7cf0fcfe7725026f5c8f1542fb"

  workspace_name      = module.config[each.key].names.avd-workspace
  dag_name            = module.config[each.key].names.avd-dag
  vm_hostname_prefix  = module.config[each.key].names.avd-host
